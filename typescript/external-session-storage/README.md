...
| Sample | Backend | Run |
| --- | --- | --- |
| [`redis-sample.ts`](redis-sample.ts) | Redis lists and indexes | `npm run redis -- /path/to/repository` |
| [`postgres-sample.ts`](postgres-sample.ts) | PostgreSQL JSONB rows | `npm run postgres -- /path/to/repository` |
...
```bash
npm run redis -- /path/to/repository
```
...
`redis-sample.ts` is independently copyable. It needs
`@qoder-ai/qoder-agent-sdk` and `ioredis`, and stores:
...
```bash
npm run postgres -- /path/to/repository
```
...
`postgres-sample.ts` is independently copyable. It needs
`@qoder-ai/qoder-agent-sdk`, `pg`, and `@types/pg`. The adapter creates
`qoder_session_entries`: each transcript entry is one JSONB row, the increasing
`seq` column preserves replay order, and the
...
history for one key in append order. `listSessions()`, `listSubkeys()`, and
...
- monitor `system/mirror_error`; the conversation can succeed even when an
